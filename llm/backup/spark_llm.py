...
from ALGPT.llm.backup.self_llm import Self_LLM
import json
import requests
...
class Spark_LLM(Self_LLM):
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None,
        ...
        # 发起请求
        try:
            response = spark_main(self.appid, self.api_key, self.api_secret,
                                  self.url, self.domain, question, self.temperature, self.max_tokens)
            return response
...
def run(ws, *args):
    data = json.dumps(gen_params(appid=ws.appid, domain=ws.domain,
                      question=ws.question, temperature=ws.temperature, max_tokens=ws.max_tokens))
    ws.send(data)
